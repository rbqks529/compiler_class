...
/* ✨ 함수 호출 인자들을 레지스터에 채우는 헬퍼
   args  : AST_STMT_LIST 로 연결된 인자 expr 들 (또는 단일 expr / NULL)
   index : 현재까지 채운 인자 개수 (0부터 시작)
   반환값: 다음에 쓸 인덱스 (채운 개수) */
static int gen_call_args(AST* args, FILE* out, int index) {
    if (!args) return index;

    if (args->type == AST_STMT_LIST) {
        index = gen_call_args(args->left, out, index);
        index = gen_call_args(args->right, out, index);
        return index;
    }

    /* 단일 expr 인자 */
    if (index >= 6) {
        fprintf(stderr, "too many arguments in function call (only up to 6 supported)\n");
        /* 그래도 expr는 평가해서 부수 효과를 유지 */
        gen_expr(args, out);
        return index + 1;
    }
    ...
    case 3:
        ...
        break;
    case 4:
        fprintf(out, "    movl %%eax, %%r8d\n");
        break;
    case 5:
        fprintf(out, "    movl %%eax, %%r9d\n");
        break;
    }
